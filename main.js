...
const createScene = () => {
  ...
  */
  ...
  var player = sphere;
  movingControlType2(camera, player, 0.04, 20, 1);
  ...
  ////////////////////////////////////////////////////////////////////////
  //changing color basic info
  let myChangingColor = new BABYLON.Color3(0, 1, 0);
  myChangingColor.r = 0;
  let colorDelta = 0.002;
  let myChangingAlpha = 0.1;
  //import mesh
  BABYLON.SceneLoader.ImportMesh("", '', 'door.obj', scene, function(newMeshes) {
    ...
    for (let i = 0; i < newMeshes.length; i++) {
      ...
      movingControlType3(newMeshes[i], 0.04, 20, 1);
    }


    //material setting
    let myMaterial = new BABYLON.StandardMaterial("myMaterial", scene);
    myMaterial.diffuseColor = new BABYLON.Color3(0.6, 1, 1);
    myMaterial.specularColor = new BABYLON.Color3(0.5, 0.5, 0.5);
    myMaterial.ambientColor = new BABYLON.Color3(0.6, 0.1, 0.1);
    myMaterial.emissive = new BABYLON.Color3(0.6, 0.1, 0.1);
    newMeshes[1].material = myMaterial;

      //Changing color per frames
      scene.registerBeforeRender(function() {
        colorDelta = (colorDelta + 0.0005) % (6);
        //console.log(colorDelta);
        myChangingColor.b = Math.sin(colorDelta);
        myMaterial.diffuseColor = myChangingColor;
      });
